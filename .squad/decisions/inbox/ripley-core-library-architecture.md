# Decision: Core Library Architecture for ElBruno.Whisper
...
## Context
Implementing the complete core library for ElBruno.Whisper, a pure C# speech-to-text library using ONNX Runtime and Whisper models from HuggingFace.
...
- **MelSpectrogramProcessor**:
  - STFT with 400-sample FFT (25ms), 160-sample hop (10ms), Hanning window
  - Custom Cooley-Tukey radix-2 FFT implementation (no external deps)
  - 80 mel bins with triangular filterbank
  - Log transform for final features
  - Output: [1, 80, 3000] tensor (30 seconds, padded/truncated)
...
2. **ONNX Runtime**: Industry standard, well-maintained, excellent performance
3. **HuggingFace Downloader integration**: Reuses existing pattern from LocalLLMs, automatic model management
4. **Static factory pattern**: Async initialization (download + load) without constructor complexity
5. **Greedy decoding default**: Fastest, most deterministic, suitable for production transcription
6. **WhisperTinyEn default**: 75MB model enables 5-minute quick start experience

## Consequences
### Positive
- Zero native dependencies for audio processing
- Consistent API design across ElBruno libraries
- Automatic model download with progress reporting
- DI-friendly architecture
- Multi-target net8.0 and net10.0 for broad compatibility

### Negative
- Custom FFT implementation may be slower than optimized libraries (trade-off for portability)
- Initial model download required (mitigated by small default model + progress reporting)
...
- All files created in `src/ElBruno.Whisper/` with proper sub-folder organization
- Comprehensive XML documentation for public APIs
- Follows ElBruno conventions: multi-targeting, namespace consistency, MIT license
- Ready for Dallas's project scaffold integration (csproj, solution, README)
